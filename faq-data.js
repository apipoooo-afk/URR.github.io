...
<script>
  ...
  // ---------------------------
  // 2) ตั้งค่า Fuse.js
  //    ใช้เฉพาะ field 'q' ตามโครงสร้างใน faq-data.js
  // ---------------------------
  const fuseOptions = {
    keys: ["q"],      // ในไฟล์มีแค่ q กับ a
    threshold: 0.4,   // ยอมให้สะกดคล้าย ๆ กันได้
    includeScore: true
  };
  ...
  function botReply(userText) {
    ...
    if (result.length === 0) {
      return "ยังไม่มีคำตอบตรงกับคำถามนี้ในระบบค่ะ\nลองพิมพ์คำถามสั้น ๆ เช่น \"ค่าเรียนเท่าไร\", \"เรียนกี่เดือน\", \"จบแล้วทำงานอะไรได้บ้าง\" หรือทักไลน์โรงเรียนเพื่อสอบถามเจ้าหน้าที่ได้เลยค่ะ 💬";
    }

    // ใช้คำตอบจาก item แรกที่ใกล้ที่สุด
  ...
  // ทำให้ฟังก์ชัน ask มองเห็นได้จาก attribute onclick ในปุ่ม
  window.ask = ask;
